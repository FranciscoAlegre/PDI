...
export class SearchTravelComponent implements OnInit {
  ...
  animateTable(): void {
    if (this.table) {
      this.table.nativeElement.classList.remove('table-animation');
      void this.table.nativeElement.offsetWidth; // Trigger reflow
      this.table.nativeElement.classList.add('table-animation');
    }
  }
}
